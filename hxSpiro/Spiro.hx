...
class Spiro {
    ...
    public static inline function spiro_iter( s: Vector<Segment>, m: Vector<BandMatrix>
            ...
            if( cyclic ){
                ...
                BandMatrix.Copy( v, 0, v, nmat, nmat );  // TODO: Fix this
                BandMatrix.Copy( v, 0, v, 2 * nmat, nmat );
                n_invert = 3 * nmat;
                j = nmat;
            } else {
                n_invert = nmat;
...
class Spiro {
    ...
    public static inline function setupPath( src:Vector<ControlPoint>, n: Int ): Vector<Segment> {
        ...
        for (i in 0...n_seg ){
            ...
            dy = r[ i + 1 ].y - r[ i ].y;
            r[ i ].dChord = hyp( dx, dy );
            if( Math.isFinite( dx ) || Math.isFinite( dy ) || Math.isFinite( ( r[ i ].dChord = hyp( dx, dy ) )) ) return null;
            r[ i ].segment_theta = Math.atan2( dy, dx );
        }
        var ilast = n_seg - 1;
        var pType: PointType;
